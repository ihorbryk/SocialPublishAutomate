...
class Publisher_linkedin
{
	...
	public function __construct()
	{
		...
		add_action( 'post_to_linkedin', function( $message, $link, $name, $picture, $description, $token, $destination, $proxy ) {
			$curl = curl_init();
			if (!empty($proxy)) {
		...
		if ( isset( $_GET['get_linkedin_token'] ) ) {
	...
	public function publish( $message, $link, $name, $picture, $description )
	{
		...
		foreach ($this->users as $user) {
			...
			foreach ($this->groups as $group) {
				...
				wp_schedule_single_event( time() + $time_stamp, 'post_to_linkedin', array( $message, $link, $name, $picture, $description, $token, $destination, $proxy ) );
	...
	public function get_token_by_code( $id )
	{
		...
		if ( isset($account[0]) ) {
			...
			curl_setopt($curl, CURLOPT_RETURNTRANSFER,true);
			curl_setopt($curl, CURLOPT_POSTFIELDS, "grant_type=authorization_code&client_id={$account->client_id}&redirect_uri=http://". $_SERVER['HTTP_HOST'] ."/wp-admin/admin.php?page=spa_get_token.php&client_secret={$account->client_secret}&code={$account->code}");
			$out = curl_exec($curl);
			$out = json_decode( $out );
			$upload_dir_info = wp_upload_dir();
			curl_close($curl);
